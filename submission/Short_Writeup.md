...
**Candidate:** Kristball AIML Take-Home | **Date:** 21 Sep 2026
...
**What would break at scale & Production Tracking:**
* **What breaks:** 28-day moving window fails if COVID/lockdown happens or major menu swap occurs (cold start problem for new SKUs). Also, `scipy`/`pandas` memory might blow up if scaling from 6 bars to 600 bars. We'd need to port the logic to PySpark or a dbt SQL pipeline.
* **What to track:** Track per-SKU MAPE drift >15% → auto-alert DS; track service level (in-stock %), spoilage waste %, and Days of Supply on Hand (DOSH) weekly.
